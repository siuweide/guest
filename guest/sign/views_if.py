...
def add_guest(request):
    ...
    result = Guest.objects.filter(event=eid, realname=realname, phone=phone)
    if result:
        return JsonResponse({'status':10025, 'message':'guest is exists'})
    ...
    if e_time > now_time:
        return JsonResponse({'status':10026,'message':'event has started'})
    try:
        Guest.objects.create(realname=realname,phone=int(phone),email=email,sign=0,event_id=int(eid))
    except IntegrityError:
        return JsonResponse({'status':10027,'message':'the event guest phone number repeat'})
...
def get_guest(request):
    ...
    if phone == '':
        return JsonResponse({'status': 10022, 'message': 'phone cannot be empty'})
...
def get_guest_list(request):
    ...
    if request.method == 'GET':
        ...
        for guest in guest_list:
            guests = {}
            guests['gid'] = guest.id
            guests['realname'] = guest.realname
            guests['phone'] = guest.phone
...
def delete_guest(request):
    ...
    if request.method == 'POST':
        ...
        guest = Guest.objects.filter(id=gid)
        if guest:
            guest.delete()
            return JsonResponse({'status':200, 'message':'success'})
        else:
            return JsonResponse({'status':10201, 'message':'gid is not exists'})
    else:
